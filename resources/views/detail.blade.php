...
@section('content')
    ...
    <main class="main__content_wrapper">
                                            ...
                                            <span itemprop="itemListElement" itemscope
                                                ...
                                                @else
                                                    <span class="fw-medium"
                                                        style="font-size: 15px; line-height: 22px; color: #333;"
                                                        aria-current="page" itemprop="name">{{ $crumb['name'] }}</span>
                                                    <meta itemprop="position" content="{{ $index + 1 }}">
                                                @endif
                                            </span>
                                        @endforeach
                                    @else
                                        {{-- Fallback breadcrumb if SEO data not available --}}
                                        ...
                                        <span class="fw-medium" style="font-size: 15px; line-height: 22px; color: #333;"
                                            aria-current="page">{{ __('detail.product') }}</span>
                                    @endif
                                </div>
                            </nav>
                        </div>
        ...
        <section class="product__details--section section--padding" itemscope itemtype="https://schema.org/Product">
            <div class="container">
                <div class="row row-cols-1 row-cols-md-2 g-0 g-md-3">
                    <div class="col mb-3 mb-md-0">
                        <div class="product__details--media">
                            <meta itemprop="image" content="{{ $product->image ?: asset('logos/logo.png') }}">
                            @if ($product->category)
                                ...
                                <div class="swiper-wrapper">
                                    <div class="swiper-slide">
                                        <div class="product__media--preview__items"
                                            style="width: 100%; height: 350px; min-height: 300px; background-color: #f5f5f5; border-radius: 25px; position: relative;">
                                            @if ($product->image)
                                                <a class="product__media--preview__items--link glightbox product-variant-image-link variant-image-wrapper"
                                                    data-gallery="product-variant-gallery-{{ $product->id }}"
                                                    data-variant-id="{{ $product->id }}" href="{{ $product->image }}"
                                                    style="width: 100%; height: 100%; position: absolute; top: 0; left: 0; transition: opacity 0.3s ease-in-out;">
                                                    <img class="product__media--preview__items--img product-variant-image"
                                                        src="{{ $product->image }}"
                                                        alt="{{ $product->name }} - {{ __('detail.product_media_img') }}"
                                                        itemprop="image"
                                                        style="width: 100%; height: 100%; object-fit: cover; border-radius: 3px;"
                                                        loading="eager" fetchpriority="high">
                                                </a>
                                                <div class="product__media--view__icon variant-zoom-wrapper"
                                                    data-variant-id="{{ $product->id }}"
                                                    style="position: absolute; top: 10px; left: 10px; opacity: 1; transition: opacity 0.3s ease-in-out; pointer-events: auto;">
                                                    <a class="product__media--view__icon--link glightbox product-variant-zoom-link"
                                                        href="{{ $product->image }}" data-gallery="product-media-zoom">
                                                        ...
                                                        <span
                                                            class="visually-hidden">{{ __('detail.product_view') }}</span>
                                                    </a>
                                                </div>
                                            @else
                                                @foreach ($product->variants as $idx => $variant)
                                                    @if ($variant->image)
                                                        ...
                                                        <div class="product__media--view__icon variant-zoom-wrapper"
                                                            data-variant-id="{{ $variant->id }}"
                                                            style="position: absolute; top: 10px; left: 10px; opacity: {{ $idx === 0 ? '1' : '0' }}; transition: opacity 0.3s ease-in-out; {{ $idx !== 0 ? 'pointer-events: none;' : '' }}">
                                                            <a class="product__media--view__icon--link glightbox product-variant-zoom-link"
                                                                href="{{ $variant->image }}"
                    ...
                    <div class="col">
                        <div class="product__details--info w-100">
                            <form id="add-to-cart-form" action="{{ route('add-to-cart') }}" method="post">
                                ...
                                <div id="dynamic-alert" class="alert alert-dismissible fade mb-15 alert-dynamic"
                                    role="alert"
                                    style="display: none; flex-direction: row; justify-content: space-between; align-items: center; gap: 1rem;">
                                ...
                                <h1 class="product__details--info__title mb-15" itemprop="name">{{ $product->name }}</h1>
                                @if ($product->description)
                                    <div class="product__details--info__desc mb-15" itemprop="description">
                                        {{ $product->description }}
                                    </div>
                                @endif

                                @if ($product->variants->count() > 0)
                                    <div class="product__details--info__price mb-12" itemprop="offers" itemscope
                                        itemtype="https://schema.org/Offer">
                                        @php
                                            $firstVariant = $product->variants->first();
                                            $displayPrice = $firstVariant->price ?? 0;
                                            $displayCurrency = config('seo.product.default_currency', 'USD');
                                            $displayAvailability = $product->status
                                                ? 'https://schema.org/InStock'
                                                : 'https://schema.org/OutOfStock';
                                        @endphp
                                        <meta itemprop="price" content="{{ $displayPrice }}">
                                        <meta itemprop="priceCurrency" content="{{ $displayCurrency }}">
                                        <link itemprop="availability" href="{{ $displayAvailability }}">
                                        <link itemprop="url" href="{{ route('detail', $product->slug) }}">
                                        @foreach ($product->variants as $variant)
                                            <span class="current__price product-variant-price"
                                                data-variant-id="{{ $variant->id }}"
                                                style="display: {{ $loop->first ? 'inline' : 'none' }};" itemprop="price"
                                                content="{{ $variant->price }}">${{ number_format($variant->price, 2) }}</span>
                                            @if (isset($variant->original_price) && $variant->original_price > $variant->price)
                                                <span class="old__price product-variant-original-price"
                                                    data-variant-id="{{ $variant->id }}"
                                                    style="display: {{ $loop->first ? 'inline' : 'none' }};">${{ number_format($variant->original_price, 2) }}</span>
                                            @endif
                                        @endforeach
                                    </div>
                                @endif
                                                    ...
                                                    <div class="variant__color d-flex gap-1" style="flex-wrap: wrap;">
                                                        @foreach ($product->variants->pluck('color')->filter()->unique('id') as $idx => $color)
                                                            ...
                                                            <div class="variant__color--list">
                                                                <input id="color-{{ $color->id }}" name="color_id"
                                                                ...
                                                                <label class="variant__color--value"
                                                                    ...
                                                                    <div class="variant__color--value__color"
                                                                        style="background-color: {{ $color->code ?? '#cccccc' }};
                                                        display:inline-block; width:30px; height:28px; border-radius: 3px;">
                                                                    </div>
                                                                </label>
                                                            </div>
                                                        @endforeach
                                                    ...
                                                    <ul class="variant__size d-flex gap-1" style="flex-wrap: wrap;">
                                                        @foreach ($product->variants->pluck('size')->filter()->unique('id') as $idx => $size)
                                                            ...
                                                            <li class="variant__size--list">
                                                                <input id="size-{{ $size->id }}" name="size_id"
                                    ...
                                    <div class="product__variant--list quantity d-flex align-items-center mb-20">
                                        <div class="quantity__box">
                                            <button type="button"
                                                ...
                                                id="quantity-decrease-btn" aria-label="{{ __('detail.quantity_value') }}"
                                                value="{{ __('detail.decrease_value') }}">-</button>
                                            <label>
                                                <input type="number" name="quantity"
                                                    class="quantity__number quickview__value--number @error('quantity') is-invalid @enderror"
                                                    value="{{ old('quantity', 1) }}" data-counter min="1"
                                                    id="quantity-input" style="pointer-events: none;" readonly />
                                            </label>
                                            <button type="button"
                                                class="quantity__value quickview__value--quantity increase"
                                                id="quantity-increase-btn" aria-label="{{ __('detail.quantity_value') }}"
                                                value="{{ __('detail.increase_value') }}">+</button>
                                        </div>
                                        @error('quantity')
                                            <div class="text-danger small ms-2">{{ $message }}</div>
                                        @enderror
                                        <button class="primary__btn quickview__cart--btn" type="button"
                                            id="add-to-cart-btn" style="position: relative;">
                                            <span class="add-to-cart-text">{{ __('detail.add_to_cart') }}</span>
                                            <span class="add-to-cart-loading"
                                                style="display: none; position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%);">
                                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    style="animation: spin 1s linear infinite;">
                                                    <circle cx="12" cy="12" r="10" stroke="currentColor"
                                                        stroke-width="4" stroke-opacity="0.25"></circle>
                                ...
                                @if ($product->quality)
                                    <p class="product__details--info__meta--list">
                                        <strong>{{ __('detail.quality') }}</strong> <span>
                                ...
                                @if ($product->category)
                                    <p class="product__details--info__meta--list">
                                        <strong>{{ __('detail.category') }}</strong>
                                        <span itemprop="category" itemscope itemtype="https://schema.org/Thing">
                                            <span
                                                itemprop="name">{{ $product->category?->name ?? __('detail.not_available') }}</span>
                                        </span>
                                    </p>
                                @endif
                            </form>
                        ...
                        <div class="col">
                            <ul class="product__tab--one product__details--tab d-flex mb-30">
                                <li class="product__details--tab__list active" data-toggle="tab"
                                    data-target="#description">{{ __('detail.description') }}</li>
                            </ul>
                            <div class="product__details--tab__inner border-radius-10">
                                <div class="tab_content">
                                    <div id="description" class="tab_pane active show">
                                        <div
                                            class="product__tab--content flex flex-col items-center justify-center text-center">
                                            <div class="product__tab--content__step mb-30">
                                                <h2 class="product__tab--content__title h4 mb-10">{{ $product->name }}
                                                </h2>
                                            ...
                                            <div class="product__tab--content__step style2 mb-30 flex justify-center">
                                                <div class="product__tab--content__banner">
                                                    <img class="product__tab--content__banner--img border-radius-5 w-full"
                                                        src="{{ $product->placement_image }}"
                                                        alt="{{ __('detail.banner_img') }}">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const originalPriceElements = document.querySelectorAll(
                    '.product-variant-original-price[data-variant-id]');
