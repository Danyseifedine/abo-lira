...
import { createSizeColumns } from './datatable/sizeColumns';
import type { ProductSize } from './datatable/type';

// Size-specific filters interface
...
const props = defineProps<{
    sizes: DataTablePageProps<ProductSize, SizeFilters>['data'];
    filters: DataTablePageProps<ProductSize, SizeFilters>['filters'];
    columnLabels: {
        number: string;
        name_en: string;
        name_ar: string;
...
const { localFilters, isFilteringLoading, hasActiveFilters, activeFilterCount, applyFilters, clearFilters, updateFilters } = useFilters(
    initializeFilters(),
    {
        routeName: 'super-admin.product-sizes.index',
        preserveState: false,
        preserveScroll: true,
    },
);
...
const breadcrumbs: BreadcrumbItem[] = [
    {
        title: __('sidebar.product_sizes'),
        href: route('super-admin.product-sizes.index'),
    },
];

// Delete dialog using composable
const { deleteDialogOpen, itemToDelete: sizeToDelete, openDeleteDialog } = useDeleteDialog<ProductSize>();

// Create columns with delete dialog function passed as parameter
const sizeColumns = createSizeColumns(openDeleteDialog, props.columnLabels);
</script>

<template>
    <AdminLayout :breadcrumbs="breadcrumbs">
        <Head :title="__('sidebar.product_sizes')" />

        <div class="p-4">
            <div class="mb-6 flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold">{{ __('datatable.product_sizes.title') }}</h1>
                    <p class="mt-1 text-muted-foreground">{{ __('datatable.product_sizes.description') }}</p>
                </div>
                <DashboardButton variant="default" @click="() => router.get(route('super-admin.product-sizes.create'))">
                    <Plus class="mr-2 h-4 w-4" />
                    {{ __('datatable.add') }} {{ __('sidebar.product_sizes') }}
                </DashboardButton>
            </div>

            <DataTable :columns="sizeColumns" :data="sizes.data" :config="tableConfig" @selection-change="handleSelectionChange">
            ...
            <DashboardDeleteDialog
                ...
                :item-id="sizeToDelete?.id ?? null"
                :item-name="sizeToDelete?.name_en"
                route-name="super-admin.product-sizes.destroy"
