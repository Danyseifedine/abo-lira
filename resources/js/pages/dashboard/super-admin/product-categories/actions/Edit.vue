...
// Form - Initialize with category's existing values
const form = useForm({
...
// Submit handler
const submit = () => {
    form.put(route('super-admin.product-categories.update', props.category.id));
};

// Breadcrumbs
const breadcrumbs: BreadcrumbItem[] = [
    { title: __('datatable.product_categories.title'), href: route('super-admin.product-categories.index') },
    { title: __('datatable.edit_category'), href: route('super-admin.product-categories.edit', props.category.id) },
];
</script>
...
<template>
    <ActionLayout
        type="edit"
        :title="__('datatable.edit_product_category')"
        :description="__('datatable.update_the_details_below_to_modify_the_product_category')"
        :breadcrumbs="breadcrumbs"
        :back-href="route('super-admin.product-categories.index')"
        :show-card="false"
        max-width="full"
        :card-title="__('datatable.category_information')"
        :card-description="__('datatable.update_the_details_below_to_modify_the_product_category')"
    >
        <Head :title="__('datatable.edit_product_category')" />
        ...
        <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <Hint :text="__('datatable.set_whether_the_category_is_active_or_inactive')" />
            </div>
        </form>
